...
void BonusSystem::spawn(sf::Vector2f pos, BonusType type, float value) {
    Bonus b; float radius = bonusRadius(type);
    ...
    b.type = type; b.value = value; b.lifetime = BONUS_LIFETIME; b.alive = true;
    bonuses.push_back(b);
}
...
void BonusSystem::spawnOnKill(sf::Vector2f pos, float timeValue) {
    ...
    if (roll < 25) spawn(pos, BonusType::Speed, SPEED_BONUS_DURATION);
    else if (roll < 40) spawn(pos, BonusType::RapidFire, RAPID_FIRE_DURATION);
    else if (roll < 50) spawn(pos, BonusType::Pierce, PIERCE_DURATION);
}

void BonusSystem::spawnRandomPowerBonus(sf::Vector2f pos) {
    int roll = std::rand() % 3;
    if (roll == 0) spawn(pos, BonusType::Speed, SPEED_BONUS_DURATION);
    else if (roll == 1) spawn(pos, BonusType::RapidFire, RAPID_FIRE_DURATION);
    else spawn(pos, BonusType::Pierce, PIERCE_DURATION);
}

void BonusSystem::update(float dt, sf::Vector2f playerPos, float playerRadius, float& timeLeft, float& speedBoostTimer, float& rapidFireTimer, float& pierceTimer, float pickupRadius) {
    pickupPositions.clear();
    for (auto& b : bonuses) { b.lifetime -= dt; if (b.lifetime <= 0.0f) b.alive = false; }
    for (auto& b : bonuses) {
        if (!b.alive) continue; float collDist = playerRadius + b.shape.getRadius() + pickupRadius;
        if (distanceSq(b.shape.getPosition(), playerPos) < collDist * collDist) {
            if (b.type == BonusType::Time) timeLeft = std::min(TOTAL_TIME, timeLeft + b.value);
            else if (b.type == BonusType::Speed) speedBoostTimer = std::max(speedBoostTimer, b.value);
            else if (b.type == BonusType::RapidFire) rapidFireTimer = std::max(rapidFireTimer, b.value);
            else if (b.type == BonusType::Pierce) pierceTimer = std::max(pierceTimer, b.value);
...
void BonusSystem::update(float dt, sf::Vector2f playerPos, float playerRadius, float& timeLeft, float& speedBoostTimer, float& rapidFireTimer, float pickupRadius) {
    float dummyPierce = 0.0f;
    update(dt, playerPos, playerRadius, timeLeft, speedBoostTimer, rapidFireTimer, dummyPierce, pickupRadius);
}

void BonusSystem::clear() {
    bonuses.clear(); pickupPositions.clear();
}
