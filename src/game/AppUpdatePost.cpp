...
void updatePost(App& a, float rawDt) {
    ...
    eraseDead(a.bullets); eraseDead(a.enemies); eraseDead(a.particles); eraseDead(a.shockwaves); eraseDead(a.floatingTexts);
    if (a.level.consumeWaveCompleted()) { a.bullets.clear(); a.enemies.clear(); a.particles.clear(); a.shockwaves.clear(); a.floatingTexts.clear(); a.bonusSystem.clear(); a.comboCount=0; a.comboTimer=0.0f; a.spawnTimer=0.0f; }
